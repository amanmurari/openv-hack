...
Uses openenv-core base types (Action, Observation, State) so that the
environment is fully compliant with the OpenEnv specification and can
be consumed by any openenv-core-compatible RL training framework.
"""
...
# ---------------------------------------------------------------------------
# Phase and direction constants (exported for external consumers)
# ---------------------------------------------------------------------------
...
PHASE_NS_YELLOW = 3   # North-South transitioning to red (internal)
PHASE_EW_YELLOW = 4   # East-West transitioning to red (internal)
...
# ---------------------------------------------------------------------------
# Action  (extends openenv-core Action)
# ---------------------------------------------------------------------------
...
class TrafficAction(Action):
    ...
    Yellow-light transitions are handled automatically when switching between
...
# ---------------------------------------------------------------------------
# Observation  (extends openenv-core Observation which already has done/reward/metadata)
# ---------------------------------------------------------------------------
...
class TrafficObservation(Observation):
    ...
    queue_lengths:     List[int] = Field(default_factory=lambda: [0, 0, 0, 0],
                                         description="Regular vehicle count per approach")
    emergency_queue:   List[int] = Field(default_factory=lambda: [0, 0, 0, 0],
                                         description="Emergency vehicle count per approach")
    emergency_urgency: List[int] = Field(default_factory=lambda: [0, 0, 0, 0],
    ...
    # -- Penalty signals --
    total_waiting_time: float = Field(default=0.0, description="Sum of per-vehicle waiting increments this step")
    collision:          bool  = Field(default=False, description="Gridlock-induced collision flag")


# ---------------------------------------------------------------------------
# State  (extends openenv-core State which has episode_id / step_count)
# ---------------------------------------------------------------------------
...
class TrafficState(State):
    ...
    total_emergency_delay:  float = Field(default=0.0, description="Steps emergency vehicles spent waiting")
